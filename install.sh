...
INSTALL_SCRIPT_REV=3

REPO="lonsdaleite/steam-games-info"
...
zip_url="$(
  printf '%s\n' "$release_json" \
    | grep -oE "https://github.com/${REPO}/releases/download/[^\"]+steam-games-info-v[^\"]+\\.zip" \
    | head -n 1
)"
...
if ! sudo test -d "${PLUGIN_DIR}/backend/py_modules/vdf"; then
  echo "Install failed: bundled Python dependencies are missing." >&2
  exit 1
fi
